# Engineering documentation

Agent-facing, decision-dense documentation for Calorie Flow. Before code changes, run `npm run docs:list` and read documents whose `read_when` keywords match the task.

| Document | Domain |
| --- | --- |
| `auth.md` | Supabase accounts and sync |
| `agent-auth.md` | Dedicated staging account, agent login, and local visual verification |
| `database.md` | IndexedDB, Supabase schema, migrations |
| `api.md` | Next and Cloudflare API surfaces |
| `ui-patterns.md` | Client app and accessibility conventions |
| `requirements.md` | Durable product requirements from user requests |
| `i18n.md` | Current copy and localization policy |
| `CLOUD_SYNC_SETUP.md` | Deployment configuration |
| `decisions/` | Architecture Decision Records |

Keep documents short and update them alongside changes to their contracts.
