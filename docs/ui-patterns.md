...
Suggestion cards in Insights (`MaintenanceSuggestion`) offer and never apply. Nutrition data is health-adjacent, so a change to a target is always the user's to confirm, the copy attributes a missed target to the number rather than the person, and at most one card shows at a time.### Correcting a scanned food's name

`PortionSheet` (`src/features/food-capture/components/FoodEntrySheets.tsx`) lets a food's name be corrected inline — a pencil next to the name in `.selected-food` reveals a `ClearableInput` in place of the `<strong>` label. It exists because the label reader can legitimately return no product name (nothing legible on the package), and the earlier behavior of enriching from a name-similarity catalogue match sometimes attached the *wrong* product's name, picture, and even its barcode. The rename is deliberately quiet: collapsed by default, never a required field, and logging a food never waits on it. This is the same "escape hatch, not a first-class route" posture as `CustomCalorieOverride`.

Renaming has three effects, all best-effort and all skipped silently when nothing changed: the corrected name flows into the meal being logged, into the food object persisted to the local library (`onSaveFood`, newly threaded through every `PortionSheet` call site), and — only when the food came from the user's own scan (`source: "ai-label" | "custom"`, and only with a barcode) — a quiet correction to the shared `product_catalogue` row via `correctResolvedProductName`. That last step is scoped by RLS to `contributed_by = auth.uid()`: correcting a row this account did not contribute silently updates zero rows rather than erroring, and Open Food Facts / USDA rows can never be rewritten this way, because the app has no authority over public-provider data.

`PortionSheet` is keyed by `food.id` at every call site (`FoodCapture.tsx`, `TrackerApp.tsx`, `PlanView.tsx`) rather than resetting its rename draft in an effect. The project's ESLint config forbids reading or writing refs during render and flags a same-tick `setState` inside an effect as a cascading-render risk, so "reset local state when a prop's identity changes" is done by remounting via `key`, not by comparing the previous id in an effect.
