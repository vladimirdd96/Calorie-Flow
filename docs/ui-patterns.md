...
Renaming has three effects, all best-effort and all skipped silently when nothing changed: the corrected name flows into the meal being logged, into the food object persisted to the local library (`onSaveFood`, newly threaded through every `PortionSheet` call site), and — only when the food came from the user's own scan (`source: "ai-label" | "custom"`, and only with a barcode) — a quiet correction to the shared `product_catalogue` row via `correctResolvedProductName`. That last step does not require this account to be the row's original contributor: see "Who may correct a catalogue name" in `docs/database.md`. Open Food Facts / USDA rows can never be rewritten this way, because the app has no authority over public-provider data.
