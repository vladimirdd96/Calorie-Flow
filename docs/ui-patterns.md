...
`src/components/TrackerApp.tsx` is the public client entry point. `src/features/app/AppRuntime.tsx` is the intentionally thin route composition boundary; `AppShell` coordinates local-first persistence and optional sync while product UI lives in vertical slices under `src/features/`. Keep the daily log fast and understandable on a phone-sized viewport, with advanced tools in progressive disclosure.
