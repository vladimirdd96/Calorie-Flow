...
RLS: any signed-in account may read every row; inserts must set `contributed_by = auth.uid()`; direct table updates are limited to a contributor's own rows. The barcode primary key means the first correct answer wins, and confining direct updates keeps a shared table from becoming a vandalism surface. Bulk imports run with the service-role key, bypass RLS, and leave `contributed_by` null so seeded rows cannot be edited through the app at all.

#### Who may correct a catalogue name

A barcode scanned before its label could be read is saved under a generated placeholder name (`describeUnnamedProduct` in `src/lib/product-catalogue.ts`, e.g. "Scanned product 4056489814795"). Restricting name corrections to the contributor only, as the direct-update policy above does, meant that placeholder could sit wrong forever the moment its contributor moved on — even after a different account scanned the same package and read the label correctly.

`supabase/migrations/202608080001_correctable_placeholder_names.sql` adds a narrow second path: `placeholder`, a **generated column** computed from `name` itself (matching `describeUnnamedProduct`'s two output shapes), and a `SECURITY DEFINER` RPC, `correct_product_catalogue_name(p_barcode, p_name)`, that any signed-in account may call. It only ever changes `name`, and only on a row that is either the caller's own contribution or still carries a placeholder name — a product someone already properly named stays protected, exactly as before. `placeholder` being a generated column rather than a client-supplied flag matters: no client can lie about a row's eligibility by sending a fabricated boolean, since the flag is derived server-side from the stored name at read/write time, not trusted from the request. `src/lib/product-catalogue.ts`'s `correctCatalogueProduct` calls this RPC instead of a raw `.update()`; the direct-update policy stays in place as a defense-in-depth fallback, not the only path.

The `describeUnnamedProduct` output format is a contract with this migration's regex, not just UI copy — changing it requires a matching migration, or existing placeholder rows silently stop being correctable by anyone but their original contributor (closed, not open, so a mismatch fails safe).
...
Static assets were chosen over R2 and over a second Supabase project because they need no new service, no separate account and no payment method — R2 requires a card on file even at $0 usage, and Supabase Free allows two projects per organization that pause after a week of inactivity. The Workers free plan allows 20,000 files per version at 25 MiB each; the mirror uses 4,096 of them — 3.23M products, 160 MB total, averaging 40 KB per shard with the largest at 81 KB.

Products are stored as positional tuples (`PRODUCT_FIELDS` in `scripts/catalogue/shard.mjs`) rather than objects, because across three million rows the repeated JSON keys cost more than the values. Each product is bucketed by an FNV-1a hash of its canonical GTIN-14, so every equivalent spelling of one barcode reads the same single shard — a prefix-based shard would cluster badly, since GS1 prefixes group by country and company. **That hash exists in three places** (`shard.mjs`, `src/lib/product-mirror.ts`, `cloudflare/sites-worker.js`) and must stay identical; `product-mirror.test.ts` pins the builder and the Worker against each other, because divergence would fail silently as a permanent miss rather than as an error.
...
## Target and maintenance fields on the profile blob

Goal targeting adds optional fields to the `Profile` blob rather than any table: `calorieTargetMode`, `calorieTargetSetAt`, `goalWeightKg`, `maintenanceSource`, `observedMaintenanceKcal`, `observedMaintenanceUpdatedAt`, `maintenanceReviewDismissedAt` and `targetModelVersion`. Because the whole profile is one `jsonb` column in `user_profiles.data` and one `settings` row locally, this needed no SQL migration and no `DB_VERSION` bump.

`profileSchema` is `.strict()`, so every new field must be added there in the same change — an unknown key makes the entire profile unparseable and `getCloudSnapshot` throws the sync into `syncState: "error"`. That also means a client running older code will reject a profile written by a newer one until it updates; this is inherent to the strict schema and is the cost of rejecting genuinely malformed data.

`targetModelVersion` records which target model produced `calorieTarget`. A profile below `currentTargetModelVersion` is deliberately **not** recomputed on load: `normalizeCalorieTarget` leaves it alone so the target editor can show the user a one-time notice explaining that their number moved, and the recompute lands when they acknowledge it. Nothing stores the previous value — the notice compares the stored target against a freshly computed one while the version is still absent.
