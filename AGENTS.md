...
- CI visual tests use GitHub environment `agent-ui` secrets and `npm run test:visual`; they build the app against the isolated staging project and upload narrow/desktop screenshots as evidence. Its email, password, and any session must never appear in commands, logs, documentation, or source.
