...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const requestPayload = {
      messages: [
        {
          ...
          content: [
            ...
            "Return null for productName unless the product's own name is actually legible in a photo. Do not infer it from the nutrition values, the ingredients, the packaging style, or what the product resembles — a wrong name is worse than no name, because the app can ask the user for it but cannot tell a confident guess from a reading.",
      ...
      // Kimi vision needs the same non-thinking mode as photo messages sent to Coach.
      // Otherwise it may spend the response on reasoning instead of returning the JSON payload.
