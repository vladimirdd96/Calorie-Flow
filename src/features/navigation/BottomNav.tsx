...
export function BottomNav({ tab, onChange, onAdd, planEnabled }: { tab: AppTab; onChange: (tab: AppTab) => void; onAdd: () => void; planEnabled: boolean }) {
  return <nav className={`bottom-nav${planEnabled ? "" : " plan-hidden"}`} aria-label="Primary navigation">
    ...
    {planEnabled && <button type="button" aria-current={tab === "plan" ? "page" : undefined} className={tab === "plan" ? "active" : ""} onClick={() => onChange("plan")}><CalendarRange /><span>Plan</span></button>}
