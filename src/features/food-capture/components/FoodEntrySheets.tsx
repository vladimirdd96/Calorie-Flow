...
export function FoodDetailsSheet({ food, meal, hideCalories, showEstimatedBadge = true, onLog, onEdit, onEditEntry }: { food: Food; meal?: Meal; hideCalories: boolean; showEstimatedBadge?: boolean; onLog?: () => void; onEdit?: () => void; onEditEntry?: () => void }) {
  ...
  return <div className="food-details-sheet">
    ...
    <div className={`food-details-identity${meal ? " diary-entry-identity" : ""}`}><FoodAvatar food={food} /><div><strong>{food.name}</strong><span>{meal ? food.servingLabel || `${meal.amount} ${formatUnit(meal.unit, meal.amount)}` : foodSourceLabel(food)}</span>{meal && <small>Logged as {meal.amount} {formatUnit(meal.unit, meal.amount)} · {round(meal.grams, 1)} g</small>}</div>{onEdit && <button type="button" className="icon-button ghost food-edit-trigger" onClick={onEdit} aria-label={`Edit ${food.name}`}><Pencil size={18} /></button>}</div>
    {meal && <div className="food-details-context"><span>Logged in {mealLabels[meal.mealType]}</span>{meal.estimated && showEstimatedBadge && <span className="estimate-pill">Estimated</span>}</div>}
    ...
    <div className="detail-footnote"><Info size={15} /> {food.nutrientsPer100.micronutrients ? "Micronutrients are available from the label or catalogue data." : "Micronutrients were not provided for this food."}</div>
...
const foodEditorTabs = [
  { id: "details", label: "Details" },
  { id: "photo", label: "Photo" },
  { id: "nutrition", label: "Nutrition" },
] as const;
type FoodEditorTab = (typeof foodEditorTabs)[number]["id"];
...
export function FoodEditor({ food, hideCalories, onSave, onClose }: { food: Food; hideCalories: boolean; onSave: (food: Food) => void; onClose: () => void }) {
  ...
  const [tab, setTab] = useState<FoodEditorTab>("details");
  ...
  const chooseImage = (file: File | undefined) => {
    if (!file) return;
    if (!file.type.startsWith("image/") || file.size > 8_000_000) { setTab("photo"); setError("Choose an image under 8 MB."); return; }
    void readFoodImage(file).then((imageUrl) => setDraft((current) => ({ ...current, imageUrl }))).catch((caught) => { setTab("photo"); setError(caught instanceof Error ? caught.message : "The image could not be added."); });
  };
  const submit = (event: FormEvent) => {
    event.preventDefault();
    const nutritionValues = [draft.nutrientsPer100.calories, draft.nutrientsPer100.protein, draft.nutrientsPer100.carbs, draft.nutrientsPer100.fat, draft.nutrientsPer100.fiber, draft.nutrientsPer100.sugar];
    if (!draft.name.trim()) { setTab("details"); setError("Add a food name."); return; }
    if (nutritionValues.some((value) => !Number.isFinite(value) || value < 0)) { setTab("nutrition"); setError("Use zero or positive nutrition values."); return; }
    onSave({ ...draft, name: draft.name.trim(), brand: draft.brand?.trim() || undefined, barcode: draft.barcode?.trim() || undefined });
  };
  return (
    <form className="sheet-form food-editor" onSubmit={submit}>
      <div className="sheet-header"><div><span className="eyebrow">Food library</span><h2>Edit food</h2></div><span /></div>
      <div className="segmented three editor-tabs" role="tablist" aria-label="Edit food sections">
        {foodEditorTabs.map((item) => <button type="button" key={item.id} role="tab" aria-selected={tab === item.id} className={tab === item.id ? "active" : ""} onClick={() => setTab(item.id)}>{item.label}</button>)}
      </div>
      <div className="editor-tab-panel" role="tabpanel">
        {tab === "details" && <div className="editor-tab-fields">
          <label><span>Food name</span><ClearableInput autoFocus value={draft.name} maxLength={120} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} onClear={() => setDraft((current) => ({ ...current, name: "" }))} placeholder="e.g. Homemade meatballs" clearLabel="Clear food name" /></label>
          <div className="form-grid two"><label><span>Brand <small>optional</small></span><ClearableInput value={draft.brand || ""} onChange={(event) => setDraft((current) => ({ ...current, brand: event.target.value }))} onClear={() => setDraft((current) => ({ ...current, brand: "" }))} placeholder="e.g. Acme" clearLabel="Clear brand" /></label><label><span>Barcode <small>optional</small></span><ClearableInput inputMode="numeric" value={draft.barcode || ""} onChange={(event) => setDraft((current) => ({ ...current, barcode: event.target.value }))} onClear={() => setDraft((current) => ({ ...current, barcode: "" }))} placeholder="e.g. 3800123456789" clearLabel="Clear barcode" /></label></div>
        </div>}
        {tab === "photo" && <section className="food-photo-editor" aria-labelledby="food-photo-heading">
          <div><span className="eyebrow" id="food-photo-heading">Food photo</span><small>Stored privately with your food.</small></div>
          {draft.imageUrl ? <div className="food-photo-preview"><img src={draft.imageUrl} alt="Preview of this food" /><button type="button" className="secondary-button" onClick={() => inputRef.current?.click()}>Replace photo</button><button type="button" className="text-button muted" onClick={() => setDraft((current) => ({ ...current, imageUrl: undefined }))}>Remove</button></div> : <button type="button" className="food-photo-upload" onClick={() => inputRef.current?.click()}><Camera size={18} /><span><strong>Add a food photo</strong><small>Choose from your device</small></span></button>}
          <input ref={inputRef} type="file" accept="image/*" hidden onChange={(event) => { chooseImage(event.target.files?.[0]); event.target.value = ""; }} />
        </section>}
      ...
      <div className="sheet-actions editor-sticky-actions"><button type="button" className="secondary-button" onClick={onClose}>Cancel</button><button type="submit" className="primary-button"><Check size={17} />Save food</button></div>
    </form>
  );
}
...
export function QuickMacroSheet({ initialLoggedDate = localDateKey(), onLog, onClose, hideCalories }: { initialLoggedDate?: string; onLog: (meal: Meal, food: Food) => void; onClose: () => void; hideCalories: boolean }) {
  const [name, setName] = useState("");
  ...
  return <form className="sheet-form quick-macro-sheet" onSubmit={submit}>
    ...
    <button className="primary-button full" type="submit"><Plus size={18} />Log food</button>
  </form>;
}
