...
import { ArrowLeft, Camera, Check, ChevronRight, Info, Package, Pencil, Plus, Sparkles, CalendarDays } from "lucide-react";
...
export function FoodEditor({ food, hideCalories, onSave, onClose }: { food: Food; hideCalories: boolean; onSave: (food: Food) => void; onClose: () => void }) {
  ...
  const chooseImage = (file: File | undefined) => {
    ...
    const reader = new FileReader(); reader.onload = () => typeof reader.result === "string" && setDraft((current) => ({ ...current, imageUrl: reader.result as string })); reader.onerror = () => setError("The image could not be added."); reader.readAsDataURL(file);
  };
...
export function PortionSheet({ food, questions, initialMealType, onLog, onClose, hideCalories }: { food: Food; questions?: string[]; initialMealType?: MealType; onLog: (meal: Meal, food: Food) => void; onClose: () => void; hideCalories: boolean }) {
  ...
  return (
    <form className="portion-sheet" onSubmit={log}>
      ...
      <label className="meal-date-field"><span>Meal date <small>Tap to change</small></span><span className="meal-date-action"><CalendarDays size={16} aria-hidden="true" /><input type="date" aria-label="Meal date. Tap to change the day this food is logged" value={loggedDate} max={localDateKey()} onChange={(event) => setLoggedDate(event.target.value || localDateKey())} /></span></label>
