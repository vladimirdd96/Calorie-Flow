...
export function AddFoodSheet({ foods, meals, recipes, initialView = "start", initialMealType, initialLoggedDate = localDateKey(), onLog, onMealPhoto, onSaveFood, onSelectRecipe, onSelectFood, onClose, hideCalories, purpose = "log", mealTimeBoundaries, servingOverrides }: { foods: Food[]; meals: Meal[]; recipes: Recipe[]; initialView?: AddView; initialMealType?: MealType; initialLoggedDate?: string; onLog: (meal: Meal, food: Food) => void; onMealPhoto: (meal: Meal) => void; onSaveFood: (food: Food) => Promise<void>; onSelectRecipe: (recipe: Recipe) => void; onSelectFood?: (food: Food) => void; onClose?: () => void; hideCalories: boolean; purpose?: AddFoodPurpose; mealTimeBoundaries?: MealTimeBoundaries; servingOverrides?: { tbspGrams?: number; tspGrams?: number } }) {
  ...
  const runSearch = useCallback(async (value: string) => {
    ...
    try {
      ...
      if (requestId === searchRequestRef.current && !matchingRecipes.length && !matchingDiaryRecipes.length && !matchingPersonalFoods.length && !matchingReferenceFoods.length && !matchingDiaryFoods.length) setSearchError("Online food search is unavailable. You can still add a custom food.");
    } finally {
  ...
  if (view === "label") return <LabelReader knownBarcode={unknownBarcode} initialFiles={pendingImages} onFood={(food, followUps) => { void handleLabelFood(food, followUps); }} onClose={() => { setPendingImages([]); changeView("start"); }} />;
  ...
  if (view === "search") return (
    <div>
      ...
      <form className="sheet-search" onSubmit={search}><Search size={19} /><ClearableInput autoFocus value={query} onChange={(event) => setQuery(event.target.value)} onClear={() => setQuery("")} placeholder="Food, recipe, brand or barcode" clearLabel="Clear food search" /><button type="submit">Search now</button></form>
