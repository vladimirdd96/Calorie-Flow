...
export function RecipeLogSheet({ recipe, foods, meals, onSaveFood, onLog, onSaveEdit, editingMeal, onClose }: { recipe: Recipe; foods: Food[]; meals: Meal[]; onSaveFood: (food: Food) => Promise<void>; onLog: (meal: Meal) => Promise<void>; onSaveEdit?: (meal: Meal, recipe: Recipe) => Promise<void>; editingMeal?: Meal; onClose: () => void }) {
  ...
  const removeIngredient = (ingredientId: string) => setIngredients((current) => current.filter((ingredient) => ingredient.id !== ingredientId));
  ...
  const log = async (event: FormEvent) => {
    ...
    if (editingMeal && onSaveEdit) {
      ...
      await onSaveEdit({ ...editingMeal, mealType, nutrition, imageUrl: imageUrls[0] }, nextRecipe);
    } else {
      await onLog({ id: `recipe-${crypto.randomUUID()}`, recipeId: recipe.id, recipeLogId: logId.current, name: recipe.name, mealType, amount: 1, unit: "serving", grams: 100, nutrition, imageUrl: imageUrls[0], createdAt: new Date().toISOString(), loggedDate: localDateKey(), source: "custom" });
    }
    onClose();
  };
