...
export function FastingTracker({ profile, onSave, compact = false }: { profile: Profile; onSave: (profile: Profile) => void; compact?: boolean }) {
  ...
  return <details className={`fasting-tracker rhythm-card${compact ? " fasting-pill" : " card"}${active ? " active" : ""}`} open={expanded} onToggle={(event) => setExpanded(event.currentTarget.open)}>
    <summary className="fasting-summary"><span className="rhythm-icon fasting"><Timer size={18} /></span><span><span className="eyebrow">{compact ? "Fasting" : "Automatic rhythm"}</span><strong id="fasting-heading">{active ? `${elapsed.toFixed(1)} / ${goal} h` : `${goal}-hour window`}</strong><small>{active ? "Since your last meal" : "Starts after your last meal"}</small></span><ChevronDown size={18} aria-hidden="true" /></summary>
