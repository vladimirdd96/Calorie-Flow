...
function WeightSheet({ profile, onSave, onOpenHistory }: { profile: Profile; onSave: (profile: Profile) => void; onOpenHistory: () => void }) {
  ...
  const save = () => {
    ...
    const weightKg = imperial ? lbToKg(typed) : typed;
  ...
  >
    ...
    <div className="today-weigh-row">
      <label><span className="visually-hidden">Weight in {imperial ? "pounds" : "kilograms"}</span><input autoFocus inputMode="decimal" value={draft} onChange={(event) => setDraft(event.target.value)} /><small>{unit}</small></label>
