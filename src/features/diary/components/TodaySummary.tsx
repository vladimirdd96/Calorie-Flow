...
function NutritionExpanded({ total, targets }: { total: Nutrition; targets: DailyTargets }) {
  ...
  return <div className="today-nutrition-expanded">
    ...
    <div className="macro-share-labels"><span>{shares.protein}% protein</span><span>{shares.carbs}% carbs</span><span>{shares.fat}% fat</span></div>
    <div className="expanded-targets">
      {([["protein", total.protein, targets.protein, "var(--protein)"], ["carbs", total.carbs, targets.carbs, "var(--carbs)"], ["fat", total.fat, targets.fat, "var(--fat)"]] as const).map(([label, value, target, color]) => <div key={label} style={{ "--macro-color": color } as CSSProperties}><i /><span><b style={{ width: `${Math.min(100, value / Math.max(1, target) * 100)}%` }} /></span><strong>{round(value)} / {target}g</strong></div>)}
    </div>
    <div className="nutrition-chip-row"><span className="blue">Fibre {round(total.fiber)} / {targets.fiber}g</span><span className="amber">Sugar {round(total.sugar)}g</span><span className="green">{Math.max(0, Math.round(targets.calories - total.calories))} kcal left</span></div>
