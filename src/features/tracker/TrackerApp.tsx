  ...
  return (
    <div className="app-shell">
      ...
      <div className="content-shell" inert={modalOpen} aria-hidden={modalOpen || undefined}>
        ...
        {tab === "plan" && profile.planEnabled && <PlanView profile={profile} foods={foods} meals={meals} onSaveFood={saveFood} onSave={(next) => void saveProfile(next)} onLog={saveNewMeal} />}
      ...
      {detailMeal && <Sheet onClose={() => setDetailMeal(undefined)} wide label={`Food details for ${detailMeal.name}`}><FoodDetailsSheet food={foods.find((food) => food.id === detailMeal.foodId) || { id: detailMeal.foodId || detailMeal.id, name: detailMeal.name, brand: detailMeal.brand, imageUrl: detailMeal.imageUrl, nutrientsPer100: detailMeal.nutrition, source: detailMeal.source }} meal={detailMeal} hideCalories={profile.hideCalories} onEditEntry={() => { setDetailMeal(undefined); setEditingMeal(detailMeal); }} onEdit={() => { setFoodDetails(foods.find((food) => food.id === detailMeal.foodId) || { id: detailMeal.foodId || detailMeal.id, name: detailMeal.name, brand: detailMeal.brand, imageUrl: detailMeal.imageUrl, nutrientsPer100: detailMeal.nutrition, source: detailMeal.source }); setDetailMeal(undefined); }} /></Sheet>}
      {foodDetails && <Sheet onClose={() => setFoodDetails(undefined)} wide label={`Food details for ${foodDetails.name}`}><FoodDetailsSheet food={foodDetails} hideCalories={profile.hideCalories} onEdit={() => { setEditingFood(foodDetails); setFoodDetails(undefined); }} onLog={() => { setDirectFood(foodDetails); setFoodDetails(undefined); setAdding(true); }} /></Sheet>}
      {editingFood && <Sheet onClose={() => setEditingFood(undefined)} wide label={`Edit food ${editingFood.name}`}><FoodEditor food={editingFood} hideCalories={profile.hideCalories} onSave={(food) => { void saveFood(food); setEditingFood(undefined); }} onClose={() => setEditingFood(undefined)} /></Sheet>}
