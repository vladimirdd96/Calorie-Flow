...
export function PlanSlotCard({ mealLabel, entry, recipe, food, onRemove, onAdd }: {
  ...
  return <article className="plan-slot-card">
    <div className="plan-slot-heading">
      <span>{mealLabel}</span>
      {entry && <button type="button" aria-label={`Remove ${mealLabel} plan`} onClick={onRemove}><X size={14} /></button>}
    </div>
    ...
    </div> : <button type="button" className="plan-slot-add" onClick={onAdd}><Plus size={13} />Add to plan</button>}
  </article>;
}
