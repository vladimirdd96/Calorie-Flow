...
import type { Meal, MealType, Profile, Recipe } from "@/lib/types";
...
function RecipeComposer({ recipe, onSave }: { recipe?: Recipe; onSave: (recipe: Recipe) => void }) {
  ...
  const addImages = async (files: FileList | null) => {
    ...
    setImageError("");
    try { const remaining = 8 - imageUrls.length; if (remaining <= 0) throw new Error("A recipe can include up to 8 photos."); const images = await Promise.all([...files].slice(0, remaining).map(readMealImage)); setImageUrls((current) => [...current, ...images]); } catch (error) { setImageError(error instanceof Error ? error.message : "The photo could not be added."); }
  };
...
export function PlanView({ profile, onSave, onLog }: { profile: Profile; onSave: (profile: Profile) => void; onLog: (meal: Meal) => Promise<void> }) {
  ...
  return <main className="page plan-page">
    ...
    {loggingRecipe && <Sheet onClose={() => setLoggingRecipe(undefined)} label={`Log ${loggingRecipe.name}`} wide><RecipeLogSheet recipe={loggingRecipe} onLog={onLog} onClose={() => setLoggingRecipe(undefined)} /></Sheet>}
