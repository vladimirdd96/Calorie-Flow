...
import type { Food, Meal, MealPlanEntry, MealType, Profile, Recipe } from "@/lib/types";
...
export function PlanView({ profile, foods, meals, onSaveFood, onSave, onLog, onOpenFoods }: { profile: Profile; foods: Food[]; meals: Meal[]; onSaveFood: (food: Food) => Promise<void>; onSave: (profile: Profile) => void; onLog: (meal: Meal) => Promise<void>; onOpenFoods: () => void }) {
  ...
  const [planningMeal, setPlanningMeal] = useState<MealType>();
  ...
  const addPlanEntry = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const [kind, itemId] = planItemId.split(":");
    if (!itemId || (kind !== "recipe" && kind !== "food")) return;
    const entry: MealPlanEntry = kind === "recipe" ? { id: `plan-${crypto.randomUUID()}`, recipeId: itemId, date, mealType } : { id: `plan-${crypto.randomUUID()}`, foodId: itemId, date, mealType };
    onSave({ ...profile, mealPlanEntries: [...(profile.mealPlanEntries || []), entry] });
    setPlanningMeal(undefined);
    setPlanItemId("");
  };
  ...
  return <main className="page plan-page">
    ...
    {planningMeal && <Sheet label={`Add to ${mealLabels[planningMeal]}`} onClose={() => setPlanningMeal(undefined)}><form className="plan-picker-sheet" onSubmit={addPlanEntry}><div className="sheet-header"><div><span className="eyebrow">Add to plan</span><h2>{mealLabels[planningMeal]}</h2></div><span /></div>{recipes.length || foods.length ? <><label><span>Recipe or food</span><ThemedSelect ariaLabel="Recipe or food to plan" value={planItemId} onChange={setPlanItemId} options={[{ value: "", label: "Choose a recipe or food" }, ...recipes.map((recipe) => ({ value: `recipe:${recipe.id}`, label: `Recipe · ${recipe.name}` })), ...foods.map((food) => ({ value: `food:${food.id}`, label: `Food · ${food.name}` }))]} /></label><button className="primary-button full" type="submit" disabled={!planItemId}><Plus size={17} />Add to plan</button></> : <div className="recipe-empty"><strong>No recipes or foods yet</strong><p>Save a recipe or food first, then add it to your week.</p><button type="button" className="secondary-button" onClick={() => { setPlanningMeal(undefined); setSection("recipes"); }}>Open recipes</button></div>}</form></Sheet>}
