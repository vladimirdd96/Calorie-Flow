...
export function PlanView({ profile, foods, meals, onSaveFood, onSave, onLog, onOpenFoods }: { profile: Profile; foods: Food[]; meals: Meal[]; onSaveFood: (food: Food) => Promise<void>; onSave: (profile: Profile) => void; onLog: (meal: Meal) => Promise<void>; onOpenFoods: () => void }) {
  ...
  return <main className="page plan-page">
    ...
    {section === "week" && <section id="plan-week-panel" role="tabpanel" aria-labelledby="plan-week-tab" className="planning-workspace workspace-panel"><div className="plan-day-switcher"><button type="button" aria-label="Previous day" onClick={() => movePlanDay(-1)}><ChevronLeft size={16} /></button><button type="button" className="plan-day-label" onClick={() => setPlanCalendarOpen(true)}><CalendarDays size={15} />{planDayLabel}</button><button type="button" aria-label="Next day" onClick={() => movePlanDay(1)}><ChevronRight size={16} /></button></div><div className="plan-slot-list">{(Object.keys(mealLabels) as MealType[]).map((type) => { const entry = dayEntries.find((item) => item.mealType === type); const recipe = entry ? recipes.find((item) => item.id === entry.recipeId) : undefined; return <article className="plan-slot-card" key={type}><div className="plan-slot-heading"><span>{mealLabels[type]}</span>{entry && <button type="button" aria-label={`Remove ${mealLabels[type]} plan`} onClick={() => removeEntry(entry.id)}><X size={14} /></button>}</div>{recipe ? <div className="plan-slot-recipe"><span><strong>{recipe.name}</strong><small>Planned · {recipe.servings} servings</small></span><strong>{Math.round(recipe.nutritionPerServing.calories)} <small>kcal</small></strong></div> : <button type="button" className="plan-slot-add" onClick={() => { setMealType(type); setPlanningMeal(type); }}><Plus size={13} />Add to plan</button>}</article>; })}</div></section>}
    ...
    {planningMeal && <Sheet label={`Add to ${mealLabels[planningMeal]}`} onClose={() => setPlanningMeal(undefined)}><form className="plan-picker-sheet" onSubmit={addPlanEntry}><div className="sheet-header"><div><span className="eyebrow">Add to plan</span><h2>{mealLabels[planningMeal]}</h2></div><span /></div>{recipes.length ? <><label><span>Recipe</span><ThemedSelect ariaLabel="Recipe to plan" value={recipeId} onChange={setRecipeId} options={[{ value: "", label: "Choose a recipe" }, ...recipes.map((recipe) => ({ value: recipe.id, label: recipe.name }))]} /></label><button className="primary-button full" type="submit" disabled={!recipeId}><Plus size={17} />Add to plan</button></> : <div className="recipe-empty"><strong>No recipes yet</strong><p>Save a recipe first, then add it to your week.</p><button type="button" className="secondary-button" onClick={() => { setPlanningMeal(undefined); setSection("recipes"); }}>Open recipes</button></div>}</form></Sheet>}
