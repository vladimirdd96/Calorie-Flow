...
function RecipeComposer({ recipe, onSave }: { recipe?: Recipe; onSave: (recipe: Recipe) => void }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!name.trim() || !Number.isFinite(Number(servings)) || Number(servings) <= 0 || !Number.isFinite(gramsPerServing) || gramsPerServing <= 0 || ingredientNames.length === 0 || Object.values(values).some((value) => !Number.isFinite(value) || value < 0)) return;
    const now = new Date().toISOString();
    onSave({ id: recipe?.id || `recipe-${crypto.randomUUID()}`, name: name.trim(), servings: Number(servings), servingGrams: gramsPerServing, ingredients: ingredientNames.map((item, index) => ({ ...recipe?.ingredients[index], id: recipe?.ingredients[index]?.id || `ingredient-${crypto.randomUUID()}`, name: item })), nutritionPerServing: values, imageUrls, createdAt: recipe?.createdAt || now, updatedAt: now });
    setName(""); setIngredients(""); setServings("2"); setServingGrams("100"); setNutrition({ calories: "400", protein: "25", carbs: "45", fat: "12", fiber: "8", sugar: "6" });
  };
  ...
  return <form className="recipe-composer" onSubmit={submit}>
    <div className="form-grid two"><label className="span-two"><span>Recipe name</span><input required value={name} maxLength={240} onChange={(event) => setName(event.target.value)} placeholder="e.g. Weeknight lentil bowl" /></label><label><span>Servings</span><NumericInput required min="0.5" max="100" step="0.5" value={servings} onChange={(event) => setServings(event.target.value)} /></label><label><span>Grams per serving</span><NumericInput required min="1" max="20000" step="1" value={servingGrams} onChange={(event) => setServingGrams(event.target.value)} /></label><label><span>Ingredients</span><textarea required value={ingredients} maxLength={5000} onChange={(event) => setIngredients(event.target.value)} placeholder="One ingredient per line" /></label></div>
...
export function PlanView({ profile, foods, onSave, onLog, onOpenFoods, onOpenAdd }: { profile: Profile; foods: Food[]; onSave: (profile: Profile) => void; onLog: (meal: Meal) => Promise<void>; onOpenFoods: () => void; onOpenAdd: (date: string, mealType: MealType) => void }) {
